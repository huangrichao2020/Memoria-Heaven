...
export function ChatPanel() {
  ...
  const sendMessage = async (text?: string) => {
    ...
    if (!msg || !activeAgent || isTyping) return;
    ...
    try {
      ...
      for await (const chunk of activeAgent.chatStream(msg)) {
        response += chunk;
        setMessages((prev) => {
          const updated = [...prev];
  ...
  const mood = activeAgent.getMood();
    ...
    <div style={styles.container}>
        ...
        <div style={styles.headerLeft}>
          ...
          <div>
            ...
            <div style={styles.status}>
              {MOOD_EMOJI[mood.primary]} {MOOD_LABELS[mood.primary]}
            </div>
          </div>
        </div>
      ...
      <div style={styles.messages}>
        ...
        {messages.map((msg, i) => {
          ...
          if (msg.role === 'pulse') {
            return (
              <div key={i} style={styles.pulseRow}>
                <div style={styles.pulseBubble}>
                  {msg.content}
                </div>
              </div>
            );
...
const styles: Record<string, React.CSSProperties> = {
  ...
  empty: {
    ...
    justifyContent: 'center', height: '100%', color: '#666', gap: 8, textAlign: 'center',
  },
  ...
  pulseRow: { display: 'flex', justifyContent: 'center' },
  pulseBubble: {
    maxWidth: '90%', padding: '6px 12px', borderRadius: 12,
    fontSize: 12, lineHeight: 1.4, color: '#888', fontStyle: 'italic',
    background: 'rgba(42, 39, 64, 0.5)', textAlign: 'center',
  },
  quickReplies: {
  ...
  quickReplyBtn: {
    padding: '4px 10px', background: '#252236', border: '1px solid #444',
    borderRadius: 12, color: '#aaa', fontSize: 12, cursor: 'pointer',
    whiteSpace: 'nowrap',
  },
  inputRow: {
    display: 'flex', padding: '12px 16px', borderTop: '1px solid #333',
    gap: 8, background: '#252236',
  },
  input: {
